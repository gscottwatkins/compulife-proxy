...
// ── POST /scan-lead — IQ Scanner Pro (native fetch, multi-page support) ──
app.post('/scan-lead', async (req, res) => {
  // Accepts single: { file, mediaType }
  // OR multi-page:  { files: [{data, mediaType}, ...] }
  const { file, mediaType, files } = req.body;
  if (!file && (!files || !files.length)) {
    return res.status(400).json({ error: 'No file provided' });
  }
  try {
    // Build content array — support multiple pages/images
    const imageBlocks = [];
